...
def get_benchmarks(benchmark_names: List[str]) -> List[Benchmark]:
    benchmarks = []
    benchmark_registry_path = os.path.join(project_root, "playpen_benchmarks", "benchmark_registry.json")
    ...
    for name in benchmark_names:
        benchmark_entry = next((entry for entry in benchmark_registry if entry.get('benchmark_name') == name),
                               None)
        b_cls = getattr(functional, benchmark_entry["benchmark_class"])  # Creates an instance of my_module.MyClass
